...
QtGui.QApplication.addLibraryPath(CGTW_ROOT + u"/bin/lib/pyside/PySide2/plugins")
...
class CGTWWapper(cgtw2.tw):
    ...
    def w_pub(self, pub_data):
        ...
        return self.send_local_http(self.w_database, self.w_module,
                                        "filebox_bulk_upload_to_filebox",
                                        t_dict,
                                        "get")
...
def filter_data(filebox_data_list, pub_sign):
    """
    筛选出指定标识的文件框数据
...
def start_pub():
    ...
    # 获取所有 filebox 数据
    filebox_data_list = cgtww.w_get_filebox_data()
    if not filebox_data_list:
        return
    # 获取 pub_sign 参数
    pub_sign = cgtww.client.get_argv_key(u"pub_sign")
    # 筛选数据
    filebox_data_list = filter_data(filebox_data_list, pub_sign)
    for fd in filebox_data_list:
        # 获取提交信息
        CFI = VersionFilesInfo(a_tw=cgtww,
                               a_database=cgtww.w_database,
                               a_module=cgtww.w_module,
                               a_module_type=cgtww.w_module,
                               a_id_list=cgtww.w_id,
                               a_filebox_id=fd[u"#id"],
                               a_func=cgtww.task)
        # 提交
        pprint.pprint(CFI.get())
        cgtww.w_pub(CFI.get())
    # 刷新
    cgtww.w_refresh()
